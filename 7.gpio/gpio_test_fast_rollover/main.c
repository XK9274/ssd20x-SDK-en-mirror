...
int main(int argc, char **argv)
{
    ...
    addr = (unsigned long)(map_base + BANK * 0x200 + OFFSET * 4);
     //The test frequency can reach 21.6M Hz
     while (1)
     {
         *(unsigned short *)addr = CONTENT_L;
         //usleep(1000); //When the high-precision clock is not turned on, the minimum usleep time is 10ms. If you want to control the flip frequency through usleep, you need to turn on the high-precision clock.
     ...
     system("echo 11 > /sys/class/gpio/export");
      ...
      //The test frequency can only reach 172Hz
      while (1)
      {
